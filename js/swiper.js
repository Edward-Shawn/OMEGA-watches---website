   ...
   $(window).resize(function(){
    var windowWidth = $(window).width();
      if(windowWidth < 625){
        ...
        $("#slides").attr('id','slidesOff');
        $(".stop-slide").css('display','block');
      }
   });

    var sliding = startClientX = startPixelOffset = pixelOffset = currentSlide1 = 0;
    ...
    */
    function slide(event) {
      ...
      if (sliding == 2) {
        ...
        if ((currentSlide1 == 0 && event.clientX > startClientX) ||
           (currentSlide1 == slideCount - 1 && event.clientX < startClientX))
    ...
    */
    function slideEnd(event) {
      if (sliding == 2){
        ...
        currentSlide1 = pixelOffset < startPixelOffset ? currentSlide1 + 1 : currentSlide1 -1;
        // Make sure that unexisting slides weren't selected.
        currentSlide1 = Math.min(Math.max(currentSlide1, 0), slideCount - 1);
        // Since in this example slide is full viewport width offset can be calculated according to it.
        pixelOffset = currentSlide1 * -$('#slides').width();
